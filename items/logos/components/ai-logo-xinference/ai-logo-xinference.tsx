/**
 * Xinference logo.
 *
 * Artwork from @lobehub/icons-static-svg — MIT licensed.
 * https://github.com/lobehub/lobe-icons
 *
 * The Xinference name and logo are trademarks of their respective owner, reproduced here to identify
 * them. Check the owner's brand guidelines before using the mark in your own product.
 */

import type * as React from "react";

import { cn } from "@/lib/utils";

function AiLogoXinference({ className, ...props }: React.ComponentProps<"svg">) {
  return (
    <svg
      viewBox="0 0 24 24"
      role="img"
      aria-label="Xinference"
      className={cn("size-6", className)}
      {...props}
    >
      <path d="M5.223 9.692c.652 1.795 1.925 3.376 3.396 4.573 1.482 1.229 3.254 2.17 5.122 2.653a9.99 9.99 0 002.033.302c1.302.05 2.713-.206 3.758-1.04 1.297-1.036 1.651-2.625 1.318-4.21-.209-.993-.641-1.93-1.205-2.787a10.284 10.284 0 00-.366-.525.008.008 0 01.005-.007h.004c.002 0 .004 0 .006.002l.394.405a17.227 17.227 0 012.484 3.262c.579.993 1.023 2.046 1.255 3.144.369 1.747.07 3.546-1.306 4.777-.724.648-1.655 1.041-2.59 1.235-1.297.267-2.649.228-3.965.007-.669-.112-1.315-.26-1.937-.443-2.576-.756-5.012-2.051-7.143-3.677a20.968 20.968 0 01-3.484-3.296C1.949 12.813 1.046 11.396.487 9.853.12 8.845-.087 7.725.035 6.663c.267-2.306 1.98-3.654 4.174-4.06 1.265-.234 2.594-.186 3.879.037a17.71 17.71 0 013.978 1.192v.004a.006.006 0 01-.004.004h-.004a8.907 8.907 0 00-2.869-.29c-.807.048-1.666.263-2.357.656-1.034.588-1.67 1.463-1.907 2.625a4.567 4.567 0 00-.069 1.1c.025.58.163 1.198.367 1.761z" fill="url(#xinference-lobe-icons-xinference-0-_R_0_)">
      </path>
      <path d="M18.02 7.235a.05.05 0 01-.007.03c-.461.916-.923 1.832-1.386 2.747-.424.837-.745 1.437-.965 1.8a17.877 17.877 0 01-2.98 3.707.027.027 0 01-.03.005 12.678 12.678 0 01-4.205-2.777c-.14-.14-.28-.288-.42-.447a.024.024 0 01-.005-.013c0-.005 0-.01.003-.014a17.718 17.718 0 011.68-2.379 18.27 18.27 0 012.7-2.606c.408-.32 1.39-1.094 2.95-2.323L21.652.002a.008.008 0 01.01 0 .01.01 0 01.004.005.01.01 0 010 .006l-3.648 7.222z" fill="url(#xinference-lobe-icons-xinference-1-_R_0_)">
      </path>
      <path d="M2.027 24c.002 0 .004 0 .005-.002l5.843-4.58a.02.02 0 00.008-.017.02.02 0 00-.01-.016 26.743 26.743 0 01-2.584-1.842h-.006a.014.014 0 00-.005.002.012.012 0 00-.004.005L2.02 23.987a.01.01 0 000 .006c0 .002 0 .004.002.005a.009.009 0 00.006.002z" fill="url(#xinference-lobe-icons-xinference-2-_R_0_)">
      </path>
      <defs>
      <linearGradient gradientUnits="userSpaceOnUse" id="xinference-lobe-icons-xinference-0-_R_0_" x1=".478" x2="22.985" y1="3.451" y2="19.698">
      <stop stopColor="#6F11F4">
      </stop>
      <stop offset="1" stopColor="#AA66F1">
      </stop>
      </linearGradient>
      <linearGradient gradientUnits="userSpaceOnUse" id="xinference-lobe-icons-xinference-1-_R_0_" x1="21.676" x2="2.034" y1=".006" y2="23.987">
      <stop stopColor="#F52C77">
      </stop>
      <stop offset="1" stopColor="#E9A45F" stopOpacity=".996">
      </stop>
      </linearGradient>
      <linearGradient gradientUnits="userSpaceOnUse" id="xinference-lobe-icons-xinference-2-_R_0_" x1="21.676" x2="2.034" y1=".006" y2="23.987">
      <stop stopColor="#F52C77">
      </stop>
      <stop offset="1" stopColor="#E9A45F" stopOpacity=".996">
      </stop>
      </linearGradient>
      </defs>
    </svg>
  );
}

export { AiLogoXinference };
